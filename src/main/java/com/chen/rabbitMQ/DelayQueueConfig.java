...
public class DelayQueueConfig {
    ...
    public static final Integer CANCEL_ORDER_DELAY_TIME = 10000; //取消订单消息延时时间
    public static final Integer CHECK_ORDER_DELAY_TIME = 10000;  // 确认订单消息延时时间
    public static final Integer CANCEL_TRANSACTION_DELAY_TIME = 10000; // 取消交易消息延时时间
    ...
    public static final String CANCEL_TRANSACTION_DELAY_QUEUE = "cancel_transaction_delay_queue"; // 自动取消交易延时队列
    public static final String CANCEL_TRANSACTION_DELAY_ROUTING_KEY = "cancel_transaction_delay_routing_key"; // 自动取消交易延时routing_key
    public static final String CANCEL_TRANSACTION_DEAD_QUEUE = "cancel_transaction_dead_queue"; // 自动取消交易死信队列
    public static final String CANCEL_TRANSACTION_DEAD_ROUTING_KEY = "cancel_transaction_dead_routing_key"; // 自动取消交易死信routing_key
    ...
    * */
     ...
     * */
    @Bean
    public Queue cancelTransactionDelayQueue() {
        return QueueBuilder.durable(CANCEL_TRANSACTION_DELAY_QUEUE)
                .deadLetterExchange(DEAD_EXCHANGE)
                .deadLetterRoutingKey(CANCEL_TRANSACTION_DEAD_ROUTING_KEY)
                .ttl(CANCEL_TRANSACTION_DELAY_TIME)
                .build();
    }
    ...
    * */
     ...
     * */
    @Bean
    public Queue cancelTransactionDeadQueue() {
        return QueueBuilder.durable(CANCEL_TRANSACTION_DEAD_QUEUE).build();
    }
    ...
    * */
    ...
    /*
     * 声明自动取消交易延时队列和普通交换机的绑定关系
     * */
    @Bean
    public Binding cancelTransactionDelayQueueBindingNormalExchange(@Qualifier("cancelTransactionDelayQueue") Queue queue,
                                                             @Qualifier("normalExchange") DirectExchange exchange) {
        return BindingBuilder.bind(queue).to(exchange).with(CANCEL_TRANSACTION_DELAY_ROUTING_KEY);
    }
    ...
    * */
    ...
    /*
     * 声明自动取消交易死信队列和死信交换机的绑定关系
     * */
    @Bean
    public Binding cancelTransactionDeadQueueBindingDeadExchange(@Qualifier("cancelTransactionDeadQueue") Queue queue,
                                                          @Qualifier("deadExchange") DirectExchange exchange) {
        return BindingBuilder.bind(queue).to(exchange).with(CANCEL_TRANSACTION_DEAD_ROUTING_KEY);
    }
